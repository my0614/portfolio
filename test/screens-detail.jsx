...
function ProjectDetail({ id, goBack }) {
  ...
  return (
    <div className={"screen" + (revIn ? " in" : "")} ref={reveal} data-active>
      <AppBar title={pr.title} onBack={goBack} scrolled={scrolled}
        right={pr.link ? <a className="iconbtn" href={pr.link} target="_blank" rel="noreferrer" aria-label="link"><Icon name="link" size={22} /></a> : null} />
      <div className="scroll" ref={ref}>
        ...
        <div className="pad" style={{ paddingTop: 6, paddingBottom: 22 }}>
          <div className="rise" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <Avatar initial={pr.initial} size={36} font={15} bg={pr.type === "team" ? "var(--accent-bg)" : "var(--bg-gray)"} color={pr.type === "team" ? "var(--accent-dark)" : "var(--text-2)"} />
            <div>
          ...
          {pr.subtitle && <div className="rise" style={{ animationDelay: ".03s", fontSize: 13, fontWeight: 700, color: "var(--accent)", marginBottom: 6 }}>{pr.subtitle}</div>}
        ...
        <div className="pad rise" style={{ animationDelay: ".18s", paddingBottom: 8 }}>
          ...
          {pr.image.caption && <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 8, textAlign: "center" }}>{pr.image.caption}</div>}
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {s.intent.map((t, i) => (
              <div key={i} style={{ display: "flex", gap: 12 }}>
                <div style={{ flex: "none", width: 22, height: 22, borderRadius: 99, background: "var(--accent-bg)", color: "var(--accent-dark)", fontSize: 12, fontWeight: 800, display: "grid", placeItems: "center", marginTop: 1 }}>{i + 1}</div>
            ...
            {s.tech.map((t, i) => (
              <div key={i} style={{ border: "1px solid var(--border)", borderRadius: 16, padding: "18px 18px 16px" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <span style={{ flex: "none", fontSize: 13, fontWeight: 800, color: "var(--accent)", fontFamily: "ui-monospace, monospace", marginTop: 1 }}>{String(i + 1).padStart(2, "0")}</span>
                  ...
                  <div style={{ marginTop: 12, paddingLeft: 23, display: "flex", flexDirection: "column", gap: 8 }}>
                    {t.points.map((pt, j) => (
                      <div key={j} style={{ display: "flex", gap: 8 }}>
                        <span style={{ flex: "none", marginTop: 3, color: "var(--accent)" }}><Icon name="check" size={15} stroke={2.6} /></span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {(s.result || s.expect).map((t, i) => (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <span style={{ flex: "none", marginTop: 2, color: "var(--green)" }}><Icon name="check" size={18} stroke={2.6} /></span>
