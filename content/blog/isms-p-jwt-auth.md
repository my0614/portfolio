...
|---|---|---|---|
| **2.5.3** 사용자 인증 | 안전한 인증 방식 적용, 인증 실패 시 시도 제한 등 | mock 로그인은 딕셔너리 평문 비교(`mock["password"] != body.password`), 로그인 실패 횟수 제한 없음. OAuth 로그인은 provider에 위임되어 안전 | 부분구현 |
| **2.5.4** 비밀번호 관리 | 비밀번호는 반드시 암호화(해시)하여 저장 | `User` 모델에 `password_hash` 컬럼은 존재하나 실제로 값을 쓰거나 검증하는 코드가 없음. `bcrypt`/`passlib` 등 해싱 라이브러리 자체가 `requirements.txt`에 없음 | 미구현 |
...
| **2.7.1** 암호화 적용 | 인증정보(비밀번호, 시크릿 키 등)는 안전하게 관리 | `JWT_SECRET_KEY`는 `.env`로 분리되어 소스에 하드코딩되지 않음(양호). 반면 mock 로그인 비밀번호는 소스코드에 평문 노출(`test1234`, `password`) | 부분구현 |
| 인가(권한) 검증 | 사용자가 요청 리소스에 접근할 권한이 있는지 별도 확인 | `get_current_user`는 신원만 확인. role/권한 체크 로직 없음, 데이터 격리는 쿼리 필터링에 암묵적으로만 의존 | 미구현 |

핵심은 이거다 — **OAuth 로그인 경로는 인증 이론에 충실하게 짜여 있지만, mock 로그인과 로그아웃/세션 회수 쪽은 "일단 동작하게" 만든 티가 난다.** ISMS-P 심사에서는 정확히 이 지점, 즉 "예외 경로/부가 기능"에서 구멍이 나는 걸 잡아낸다. 실제 심사에서도 메인 플로우보다 로그아웃, 비밀번호 재설정, 관리자 계정처럼 덜 신경 쓴 기능에서 지적이 많이 나온다고 하는데, 내 코드로 직접 확인하니 왜 그런지 체감이 됐다.

## 5. 그래서 어떻게 고칠 것인가 (스터디용 설계 메모)

아래는 실제로 구현한 코드가 아니라, 위 갭을 메우기 위한 방향을 정리한 메모다.

**비밀번호 해싱 — `passlib`/`bcrypt` 도입**
...
`mock_login`의 `mock["password"] != body.password` 평문 비교를 `verify_password(body.password, user.password_hash)`로 바꾸고, 이미 존재하는 `password_hash` 컬럼을 실제로 채워 넣으면 2.5.4/2.7.1 갭이 메워진다.
...
- ISMS-P의 인증/접근통제/암호화 통제항목을 실제 코드에 대입해보면, **메인 로그인 플로우(OAuth)는 준수 수준이 괜찮지만, 비밀번호 저장, 로그아웃 시 세션 무효화, 인가 검증처럼 "부가적으로 보이는" 부분에서 갭이 크게 드러난다** — 이론으로 읽을 때보다 훨씬 명확하게 와닿았다.
