...
의약품 복약 관리 앱을 만들면서 캐시가 필요한 지점이 꽤 여러 군데 생겼다. 건강 위험 요소에 대한 LLM 조언 생성, 기상청·에어코리아 공공데이터 조회, 그리고 자체적으로 계산하는 건강 지수 — 이 세 가지를 처음엔 다 똑같이 "Redis에 넣고 TTL 걸기"로 처리했는데, 곧 이 방식이 안 맞는 지점들이 보였다.

- LLM 호출(gpt-4.1-nano로 건강 팁 생성)은 Redis 네트워크 왕복 한 번도 아까울 만큼 비싸고 느리다. 같은 요청이 짧은 시간에 반복되면 Redis까지 갈 필요도 없이 프로세스 안에서 바로 끝내고 싶었다.
- 반대로 건강 지수는 "하루 1회 생성, 이후 변경 없음"이라는 요구사항이 있는데, Redis만으로 이걸 처리하면 자정에 TTL이 지나 키가 사라지는 순간 그 기록 자체가 증발해버린다. 이건 캐시가 아니라 **영구 기록**이어야 했다.
- 날씨/미세먼지처럼 외부 API가 자주 갱신하는 데이터와, 질병 정보처럼 거의 안 바뀌는 정적 데이터를 같은 TTL로 묶어두는 것도 낭비였다.

즉 "얼마나 비싼 연산인가", "얼마나 자주 바뀌는가", "휘발돼도 되는가 영구 보존이 필요한가" — 이 세 축이 데이터마다 다 다른데, 캐시 방식을 하나로 통일하면 어떤 데이터는 과보호되고 어떤 데이터는 계속 원본을 두들기게 된다.
...
1. **L1 — 프로세스 메모리**: 네트워크 왕복이 0이다. 대신 워커가 재시작되면 사라지고, 워커가 여러 개 뜨는 배포 환경에서는 워커마다 캐시가 따로 논다는 한계를 그대로 감수했다. 그래도 같은 요청이 짧은 시간에 몰릴 때(같은 위험 요소 라벨이 여러 유저에게 반복 노출될 때) 가장 싼 계층에서 끝낼 수 있다는 이득이 컸다.
2. **L2 — Redis**: 워커 간에 공유되고, 재시작에도 살아남는다. 대신 네트워크 왕복이 붙는다.
...
- L1(프로세스 메모리)은 멀티 워커 환경에서 워커마다 따로 논다. 워커 수가 늘어날수록 L1의 실효 히트율은 떨어진다.
...
- L1(프로세스 메모리)은 공짜지만 멀티 워커 환경에서 일관성이 깨진다는 한계를 감안하고 써야 한다.
