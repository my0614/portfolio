  ...
  },
  {
    ...
    sections: {
      ...
      expect: [
        ...
        "LLM API를 단발 호출이 아닌 인증·발송·스케줄링이 결합된 운영 가능한 서비스로 구축 — LLM 기반 서비스 상용화 수준의 설계·구현 경험 확보",
        "LangChain LCEL 체인 설계로 프롬프트·LLM·파서를 독립 교체 가능한 구조화 — 향후 LangGraph 기반 멀티스텝 에이전트로 확장 가능한 기반 마련",
        "LangChain RAG 적용으로 일기가 쌓일수록 과거 감정 흐름이 반영된 개인화 편지 생성 — 단순 LLM 직접 호출 대비 맥락 연속성 확보",
      ],
    },
  },
];
