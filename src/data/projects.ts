...
export const projects: Project[] = [
  {
    ...
    tags: ["Python", "Google Sheets API", "Shopby API", "OAuth 2.0", "AWS EC2", "Cron", "Bitbucket Pipelines"],
    year: "2025.12.15 ~ 2026.03.13",
    sections: {
      아키텍처: "/projects/hotdeal-arch.svg",
      기획의도: "파트너사 담당자가 매번 수작업으로 상품을 신청하고, MD가 하루 4시간 이상을 선정·등록·결과 공유에 쏟는 구조였다. 선정 기준도 경험에 의존해 일관성이 없었고, 결과 파일도 수기로 만들어 파트너사에 개별 발송하다 보니 누락이 잦았다. 파트너사 계정 단위로 신청·선별·결과 발송까지 전 과정을 자동화해, 담당자가 기획과 판단에만 집중할 수 있는 구조를 만들었다.",
      기술: [
        {
          title: "Google Spreadsheet 기반 상품 신청 및 리스트업",
          description: "파트너사가 계정을 기준으로 Google Spreadsheet에 핫딜 상품을 신청한다. 스프레드시트 안에서 코드가 자동으로 실행되어 Shopby API와 상품명·상품번호를 매칭하고, 할인율·가격 경쟁력·재고 수량에 가중치를 부여해 상위 200개를 자동 선별한다. 선별까지의 모든 과정이 스프레드시트 내에서 완결된다.",
          points: [
            "파트너사 계정 기준으로 핫딜 신청 → Google Spreadsheet 집계",
            "Shopby API 연동으로 상품명·상품번호 자동 매칭",
            "할인율·가격 경쟁력·재고 가중치 스코어링 → 상위 200개 선별",
          ],
        },
        {
          title: "OAuth 2.0 기반 파트너사 계정별 메일 + 파일 발송",
          description: "선별 결과를 파트너사 계정별로 구분해 각 파트너사에 결과 파일을 첨부한 메일을 자동 발송한다. OAuth 2.0으로 발송 계정 인증을 처리해 수기 발송에서 발생하던 누락·오입력 리스크를 제거했다. 핫딜 신청 결과, 선정 목록, 오픈 일정을 파일 형태로 각 파트너에게 개별 전달한다.",
          points: [
            "파트너사 계정 단위로 핫딜 신청 결과·선정 목록·오픈 일정 파일 생성",
            "OAuth 2.0 인증 기반으로 파트너사별 메일 + 파일 첨부 자동 발송",
            "누락·오입력 리스크 제거, 커뮤니케이션 자동화",
          ],
        },
        {
          title: "AWS EC2 + cron 기반 실행 환경",
          description: "전체 자동화 코드는 AWS EC2 위에서 실행되며, cron으로 새벽 시간대 배치를 자동 실행해 오픈 시각 정확도를 확보했다. Bitbucket Pipelines로 코드 push 시 테스트·빌드·배포를 자동화해 수동 배포를 완전히 제거했다.",
          points: [
      ...
      성과: [
        ...
        "상품 선정부터 등록·오픈·파트너사 결과 파일 발송까지 전 과정 자동화",
